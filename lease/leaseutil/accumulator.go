...
// Consumed returns the number of resources consumed according to the requested
// resource counting strategy.
func (a *Accumulator) Consumed(strat strategy.Strategy) uint {
	switch strat {
	default:
		return a.total
	case strategy.Consumer:
		return uint(len(a.active) + len(a.released))
	}
}
